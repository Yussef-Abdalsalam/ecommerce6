    ...
    <>
        ...
        <div className="row">
          ...
          {data?.data?.data.map((el) => <div key={el.id} className=" col-xl-3 col-md-4 col-sm-6">
            <div className=" position-relative product py-1 px-3">
              <Link to={"/login"} className={`addWishList cursor-pointer fa-2x position-absolute`}><i className="fa-solid fa-heart"></i></Link>
              ...
              <div className={`${styles.btnn} text-center`}>
                <Link to={"/login"} className='btn bg-main text-white'> <i className="fa-solid fa-plus"></i> add to cart</Link>
              </div>
            </div>
          </div>)}
      ...
      <nav className='container d-flex justify-content-center' aria-label="Page navigation example">
        <ul className="pagination">
          ...
          <li className="page-item"><Link className="page-link cursor-pointer" to={"/login"} >2</Link></li>
          <li className="page-item">
            <Link className="cursor-pointer page-link" to={"/login"} aria-label="Next">
